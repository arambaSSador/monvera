...
export default async function CaseStudyPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const industryLabel = industry ? getLocalized(industry.name, locale) : cs.industry;

  return (
    <>
      <section className="relative pt-32 pb-20">
        <div className="absolute inset-0">
          <Image src={cs.image} alt={getLocalized(cs.title, locale)} fill className="object-cover" />
          <div className="absolute inset-0 bg-navy-900/85" />
        </div>
        <div className="relative container-wide mx-auto px-4">
          <Breadcrumbs
            items={[
              { label: tSections("caseStudies"), href: "/case-studies" },
              { label: getLocalized(cs.title, locale) },
            ]}
          />
          <span className="badge-accent mt-6 inline-block">{industryLabel}</span>
          <h1 className="heading-section text-white mt-4">{getLocalized(cs.title, locale)}</h1>
      ...
      <section className="section-padding">
        <div className="container-wide mx-auto max-w-4xl grid md:grid-cols-2 gap-12">
          <div>
            <h2 className="font-display font-bold text-xl text-navy-900 mb-4">{tPage("problem")}</h2>
            <p className="text-navy-600">{getLocalized(cs.problem, locale)}</p>
          </div>
          <div>
            <h2 className="font-display font-bold text-xl text-navy-900 mb-4">{tPage("solution")}</h2>
            <p className="text-navy-600">{getLocalized(cs.solution, locale)}</p>
          </div>
          <div className="md:col-span-2 bg-accent/5 rounded-3xl p-8">
            <h2 className="font-display font-bold text-xl text-navy-900 mb-4">{tPage("results")}</h2>
